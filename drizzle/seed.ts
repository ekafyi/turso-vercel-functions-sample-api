...
async function seed() {
  ...
  const storedCategories: any = await db.insert(categories).values(categoryData).returning().all();
  ...
  const storedMugs: any = await db.insert(mugs).values(mugsData).returning().all();
